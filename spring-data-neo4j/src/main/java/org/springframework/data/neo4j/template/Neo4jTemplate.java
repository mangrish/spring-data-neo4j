...
public class Neo4jTemplate implements Neo4jOperations, InitializingBean, ApplicationEventPublisherAware {
	...
	/**
	 * Set whether to expose the native Neo4j OGM Session to
	 * Neo4jCallback code.
	...
	/**
	 * Return whether to expose the native Neo4j OGM Session to
	 * Neo4jCallback code, or rather a Session proxy.
	 */
	 ...
	 * @param action callback object that specifies the Neo4j OGM action
	 * @return a result object returned by the action, or {@code null}
	 * @throws DataAccessException in case of OGM errors
	 */
	public <T> T executeWithNativeSession(Neo4jCallback<T> action) {
	 ...
	 * Neo4j OGM Session to callback code
	 * @return a result object returned by the action, or {@code null}
	 * @throws DataAccessException in case of OGM errors
	 */
	protected <T> T doExecute(Neo4jCallback<T> action, boolean enforceNativeSession) throws DataAccessException {
		...
		try {
			...
			throw SessionFactoryUtils.convertNeo4jOgmAccessException(ex);
		}
		finally {
			if (isNew) {
	...
	/**
	 * Create a close-suppressing proxy for the given Neo4j OGM Session.
	 * The proxy also prepares returned Query and Criteria objects.
	 * @param session the Hibernate Session to create a proxy for
	 * @return the Session proxy
	 * @see SessionFactory#closeSession(Session) ()
	 */
